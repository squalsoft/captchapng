...
module.exports = function captchapng(width, height, dispNumber) {
  ...
  var png = new _nodePnglib2.default(
    this.width,
    this.height,
    this.depth,
    '#FFF'
  ...
  for (var section = 0; section < this.dispNumber.length; section++) {
    ...
    var maskIndex = curNum
    if (maskIndex === '+') {
      maskIndex = 11
    }

    var mask = _font.numMask[n]
    var curMask = _font.numMask[n][maskIndex]

    var random_x_offs = parseInt(
      Math.random() * (this.widthAvg - curMask[0].length)
    )
    ...
    for (
      var _i = 0;
      _i < curMask.length && _i + random_y_offs < this.height;
      _i++
    ) {
      var lineIndex = png.index(
        this.widthAvg * section + random_x_offs,
        _i + random_y_offs
      )
      for (var j = 0; j < curMask[_i].length; j++) {
        if (
          curMask[_i][j] == '1' &&
          this.widthAvg * section + random_x_offs + j < this.width
        ) {
